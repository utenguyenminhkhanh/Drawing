...
void initgraph(void)
{
	...
	PEN_1 = getPen(PS_SOLID, 2, RGB(255, 0, 0));
...
void drawTextMau(int x, int y, const char* mess) {
	RECT rect;
	LPCWSTR message = convertCharArrayToLPCWSTR(mess);
	HWND console_handle = GetConsoleWindow();
	HDC device_context = GetDC(console_handle);
	...
	SelectObject(device_context, pen);
	SetTextColor(device_context, RGB(0, 255, 0));
	SetBkMode(device_context, TRANSPARENT);
	rect.left = x;
	rect.top = y;
	DrawText(device_context, message, -1, &rect, DT_SINGLELINE | DT_NOCLIP);
...
void lineMau(int x1, int y1, int x2, int y2)
{
	BGN.X = (short)x1 + movx;
	BGN.Y = (short)y1 + mov;
	POS.X = (short)x2 + movx;
	POS.Y = (short)y2 + mov;
	PlotLine(hDC, PEN_1, BGN, POS);
}
void huongTopo(int x1, int y1, int x2, int y2) {
	if ((x2 > x1) && y2 == y1) {
		line(x1 + 20, y1, x2 - 20, y2);
		line(x2 - 20 - 5, y2 - 5, x2 - 20, y2);
		line(x2 - 20 - 5, y2 + 5, x2 - 20, y2);
	}
	if ((x2 > x1) && y2 > y1) {
		line(x1 + 16, y1 + 16, x2 - 16, y2 - 16);
		line(x2 - 16, y2 - 16 - 5, x2 - 16, y2 - 16);
		line(x2 - 16 - 5, y2 - 16, x2 - 16, y2 - 16);
	}
	if ((x2 > x1) && y2 < y1) {
		line(x1 + 16, y1 - 16, x2 - 16, y2 + 16);
		line(x2 - 16 - 5, y2 + 16, x2 - 16, y2 + 16);
...
void circleMau(int x, int y, int radius) {
	BGN.X = (short)x + movx - radius / 2;
	BGN.Y = (short)y + mov - radius / 2;
	POS.X = (short)x + movx + radius / 2;
	POS.Y = (short)y + mov + radius / 2;
	PlotCirc(hDC, PEN_1, BGN, POS);
}
